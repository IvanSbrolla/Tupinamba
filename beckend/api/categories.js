module.exports = app => {
    ...
    const save = (req, res) => {
        ...
        if (req.query.id) category.id = req.query.id

        try {
            if (category.id) isNumeric(category.id, 'O campo ID deve ser um numero')
        ...
        if (category.id) {
            app.db('categories')
                ...
                .then(category => res.status(200).json({ "data": { category }, "err": false }))
